...
<article class="<?php echo $post_class; ?>">
	...
	<div class="post__content <?php echo ($is_first) ? 'post__content--grow-md post__content--cell-s6' : 'post__content--grow-top'; ?>">
		<?php if($is_first) : ?>
			...
			<?php the_excerpt(); ?>
			<a href="<?php the_permalink(); ?>" ui-sref="app.page({slug : '<?php echo basename(get_permalink()); ?>'})" class="post__readmore"><?php _e('Leggi tutto', 'iro'); ?><i class="icon-arrow-right"></a>
		<?php else : ?>
			<h2 class="post__title post__title--medium"><a href="<?php the_permalink(); ?>" ui-sref="app.page({slug : '<?php echo basename(get_permalink()); ?>'})"><?php the_title(); ?></a></h2>
		<?php endif; ?>
	</div>
	<?php if(!$is_first) {?><a class="post__permalink" href="<?php the_permalink(); ?>" ui-sref="app.page({slug : '<?php echo basename(get_permalink()); ?>'})"><?php _e('Leggi tutto', 'iro'); ?></a><?php } ?>
</article>