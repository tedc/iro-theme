    ...
    function iro_form_field($field, $key, $args, $value) {
        ...
        if($type == 'country') {
            ...
            if(count($options) > 1){
                ...
                $ul = '<div class="'.$base_class.'__options swiper-container" scroller="'.$key.'" options="{\'freeMode\':true, \'direction\':\'vertical\',\'mousewheel\':true,\'slidesPerView\':\'auto\', \'scrollbar\':{\'el\':\'.swiper-scrollbar\', \'draggable\':true}}"><div class="swiper-wrapper">';
                
                foreach ( $options as $skey => $svalue ) {
                    $value_selected = ($skey == $value) ? ' ng-init="'.$key.'_value=\''.addslashes($svalue).'\'"' : '';
                    $ul .= '<div class="'.$base_class.'__option swiper-slide" ng-class="{\''.$base_class.'__option--selected\':'.$args['custom_attributes']['ng-model'].'==\'' . esc_attr( $skey ) . '\'}" ng-click="'.$args['custom_attributes']['ng-model'].'=\'' . esc_attr( $skey ) . '\';'.$key.'_value=\''.addslashes($svalue).'\'"'.$value_selected.'>' . $svalue . '</div>';
                }
