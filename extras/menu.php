...
class walker_texas_ranger extends Walker_Nav_Menu {
    ...
    function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        //var_dump($item);
        $state = (basename(esc_attr($item->url)) == basename(get_home_url())) ? 'root' : 'page';
        $page = ($state == 'page') ? ', slug : \''.basename(esc_attr($item->url) ).'\'' : '';
                $menuItem =(basename(esc_attr($item->url)) == basename(get_home_url())) ? 'root' : basename(esc_attr($item->url));
...
function bem_menu($location = "main_menu", $css_class_prefix = 'main-menu', $css_class_modifiers = null, $title = null, $append = ''){
    ...
    $args = array(
        ...
        'container'         => false,
        'items_wrap'        => '<ul class="' . $css_class_prefix . ' ' . $modifiers . '"'.$title.'>%3$s'.$append.'</ul>',
