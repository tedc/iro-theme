	    ...
	    //         if(is_user_subscribed($email)) {
		   ...
		   //      }
	            ...
	            if( $mMailer->send($mEmail) && $mMailer->send($rEmail)){
	                ...
	                $data = array('formMsg' => "<h3 class='form__subtitle form__subtitle'>".__('Grazie per averci contattato', 'iro')."</h3><p>".__('Ti risponderemo nel più breve tempo possibile', 'iro')."</p><a href='".home_url('/')."' class='form__button' href='".get_home_url()."'>".__('Torna allo shop', 'iro')."</a>", 'new_nonce' => wp_create_nonce('iro-contact-form'));
	            } else {
	                ...
	                $data = array('formMsg' => "<h3 class='form__subtitle--error'>Spiacenti, qualcosa è andato storto</h3><p>".__('Si è verificato un problema con il messaggio inviato. Riprova inserendo tutti i dati correttamente oppure contattaci all\'indirizzo email', 'iro')." <a href=\'mailto:".$send_to."'>".$send_to."</a>.</p><a class='form__button' href='".get_home_url()."'>".__('Torna allo shop', 'iro')."</a>", 'new_nonce' => wp_create_nonce('iro-contact-form'));
	            }
	            wp_send_json($data);
	            wp_die();
	        else :
