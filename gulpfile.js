...
function bundle(bundler) {
    return b
      ...
      .pipe(source(path.source + 'app/main.js'))
      .pipe(buffer())
...
function fixFactorBundleMemoryLeak() {
  var listeners = b.listeners("reset").filter(l => l.name === "addHooks");
  for (var i = 0; i < listeners.length - 1; i++) { //remove "reset" listeners except last
    b.removeListener('reset', listeners[i]);
  }
}
b.on('update', bundle);
b.once('reset', fixFactorBundleMemoryLeak);
// ### Scripts
...
gulp.task('Iconfont', function(done){
    var iconStream = gulp.src([path.source + 'fonts/*.svg'])
        .pipe(iconfont({ fontName: 'iro-icons', normalize : true, formats: ['ttf', 'eot', 'woff', 'woff2', 'svg'] }));
