...
?>
<div class="shipping">
	<h2 class="shipping__subtitle"><?php echo wp_kses_post( $package_name ); ?></h2>
	<div class="shipping__packages shipping__packages--grow-md" data-title="<?php echo esc_attr( $package_name ); ?>">
		<?php if ( 1 < count( $available_methods ) ) : ?>
			<ul id="shipping_method" class="shipping__method" ng-init="checkoutFields.shipping_method[0]='<?php echo $chosen_method; ?>'">
				...
				?>
					<li class="shipping__row">
						<div class="shipping__wrapper shipping__wrapper--grow-md">
						<?php
							printf( '<input type="radio" ng-model="checkoutFields.shipping_method[%1$d]" name="shipping_method[%1$d]" data-index="%1$d" id="shipping_method_%1$d_%2$s" value="%3$s" class="shipping__radio" %4$s />
								<label for="shipping_method_%1$d_%2$s">%5$s</label>',
								$index, sanitize_title( $method->id ), esc_attr( $method->id ), checked( $method->id, $chosen_method, false ), wc_cart_totals_shipping_method_label( $method ) );
							$price = ($method->cost == 0) ? __('Gratuita', 'iro') : wc_price( $method->cost );
        					echo '<span class="shipping__price">'.$price.'</span>';
						?>
						</div>
						<?php
