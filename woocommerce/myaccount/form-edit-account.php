...
<div class="account__cell account__cell--grow-md" ng-init="isAccount = []">
	...
	<form class="account__edit account__edit--grid slide-toggle" ng-class="{'slide-toggle--visible':isAccount['details']}" method="post" ng-submit="saveAccount()">
		...
		<footer class="account__footer account__footer--s12 account__footer--grow-top">
			...
			<input type="checkbox" class="account__checkbox"<?php if($user->marketing_input) { ?> ng-init="account.marketing_input=<?php esc_attr( $user->marketing_input ); ?>"<?php } ?> ng-model="account.marketing_input" id="account_marketing_input" value="true"><label for="account_marketing_input"><span><?php _e("Acconsento all'utilizzo dei dati inseriti per l'invio di eventuali comunicazioni di marketing da parte di IRO Srl", 'iro'); ?></span></label>
