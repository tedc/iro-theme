		...
		<td class="numbers">
			<table>
				...
				<tr class="invoice-number">
					...
					<td><?php 
					$length = strlen($this->get_invoice_number());
					$prefix = '';
					if($length == 1) {
						$prefix = '00';
					} elseif($length == 2) {
					...
					$invoice_date = $this->get_date('invoice'); echo $invoice_date->date_i18n('y'); ?>.<?php echo $prefix; $this->invoice_number(); ?></td>
				</tr>
				<?php } ?>
		...
		if( sizeof( $tbody ) > 0 ) {
			foreach( $tbody as $item_id => $item_columns ) {
				...
				foreach ($item_columns as $column_key => $column_data) {
					...
					printf('<td class="%s"><div>%s</div></td>', $column_data['class'], $data);
				}
				echo '</tr>';
			}
		}
